...
import type { CateoAssistInput, CateoAssistResult, CateoInteractionArtifactPreview, CateoInteractionCheckpoint, CateoInteractionSection, CateoResponseDetail } from "./types.js";
...
export function queueConversationTurn(params: { conversationId?: string | null; viewer: Viewer; requesterId: string; ownerUserId?: string; profileId?: string; jobId: string; input: CateoAssistInput; promptText: string }) { const record = resolveConversation(params); if (record.messages.some((message) => message.role === "assistant" && (message.status === "queued" || message.status === "running"))) throw new Error("This troubleshooting thread already has a request in progress. Wait for the current evaluation to finish before replying."); const now = Date.now(); const attachments = (params.input.attachments ?? []).map((attachment) => ({ name: attachment.name, kind: attachment.kind, mimeType: attachment.mimeType, sizeBytes: attachment.sizeBytes })); const userMessage: CateoConversationMessageRecord = { messageId: crypto.randomUUID(), role: "user", text: params.promptText, createdAt: now, updatedAt: now, status: "completed", checkpoints: [], attachments }; const assistantMessage: CateoConversationMessageRecord = { messageId: crypto.randomUUID(), role: "assistant", text: "", createdAt: now, updatedAt: now, status: "queued", checkpoints: [], jobId: params.jobId }; record.messages.push(userMessage, assistantMessage); record.updatedAt = now; record.lastMessagePreview = trunc(params.promptText, 160); const saved = saveConversationRecord(record); persistRawNote({ conversationId: saved.conversationId, messageId: userMessage.messageId, createdAt: userMessage.createdAt, role: "user", title: saved.title, text: params.promptText, status: userMessage.status, input: params.input }); appendAuditEvent({ actor: "server", category: "conversation", action: "queue_turn", outcome: "success", message: `Queued conversation turn for ${saved.conversationId}`, metadata: { conversationId: saved.conversationId, jobId: params.jobId, ownerUserId: saved.ownerUserId, requesterId: saved.requesterId } }); return { conversation: saved, summary: summary(saved), userMessage, assistantMessage }; }
...
export function completeConversationTurn(params: { conversationId: string; assistantMessageId: string; result: CateoAssistResult }) { const record = loadConversationRecord(params.conversationId); if (!record) throw new Error(`Conversation ${params.conversationId} was not found`); const updated = patchAssistant(record, params.assistantMessageId, (message) => { message.status = "completed"; message.text = params.result.interaction.message; message.highlights = params.result.interaction.highlights; message.nextActions = params.result.interaction.nextActions; message.confidence = params.result.interaction.confidence; message.artifactCount = params.result.interaction.artifactCount; message.detailLevel = params.result.interaction.detailLevel; message.sections = params.result.interaction.sections; message.artifactPreviews = params.result.interaction.artifactPreviews; message.caseId = params.result.caseId; message.artifactIds = params.result.artifacts.map((artifact) => artifact.artifactId); message.checkpoints = params.result.checkpoints; message.error = undefined; }); updated.caseIds = unique([...updated.caseIds, params.result.caseId]); updated.artifactIds = unique([...updated.artifactIds, ...params.result.artifacts.map((artifact) => artifact.artifactId)]); const title = resultTitle(params.result, updated.title); updated.title = title.title; updated.titleSource = title.source; updated.lastMessagePreview = trunc(params.result.interaction.message, 160); const saved = saveConversationRecord(updated); const assistantMessage = saved.messages.find((entry) => entry.messageId === params.assistantMessageId && entry.role === "assistant"); if (assistantMessage) persistRawNote({ conversationId: saved.conversationId, messageId: assistantMessage.messageId, createdAt: assistantMessage.createdAt, role: "assistant", title: saved.title, text: assistantMessage.text, status: assistantMessage.status, caseId: params.result.caseId, artifactIds: params.result.artifacts.map((artifact) => artifact.artifactId), highlights: assistantMessage.highlights, nextActions: assistantMessage.nextActions, detailLevel: assistantMessage.detailLevel, sections: assistantMessage.sections, artifactPreviews: assistantMessage.artifactPreviews }); return saved; }
export function failConversationTurn(params: { conversationId: string; assistantMessageId: string; error: string; checkpoint?: CateoInteractionCheckpoint }) { const record = loadConversationRecord(params.conversationId); if (!record) throw new Error(`Conversation ${params.conversationId} was not found`); const saved = patchAssistant(record, params.assistantMessageId, (message) => { message.status = "failed"; message.text = "Cateo could not complete that request."; message.error = params.error; if (params.checkpoint) message.checkpoints = [...message.checkpoints, params.checkpoint]; }); const assistantMessage = saved.messages.find((entry) => entry.messageId === params.assistantMessageId && entry.role === "assistant"); if (assistantMessage) persistRawNote({ conversationId: saved.conversationId, messageId: assistantMessage.messageId, createdAt: assistantMessage.createdAt, role: "assistant", title: saved.title, text: assistantMessage.text, status: assistantMessage.status, caseId: assistantMessage.caseId, artifactIds: assistantMessage.artifactIds, error: assistantMessage.error }); return saved; }
