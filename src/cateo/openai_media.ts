...
const MAX_AUDIO_TRANSCRIPTION_BYTES = 8 * 1024 * 1024;
...
async function fetchWithTimeout(url: string, init: RequestInit, timeoutMs: number): Promise<Response> {
  ...
  try {
    ...
    if (error instanceof Error && error.name === "AbortError") {
      throw new Error(`Timed out contacting ${url}.`);
    }
    throw error;
  } finally {
...
function normalizeAudioMimeType(value: string | undefined): string {
  return value?.trim().toLowerCase() || "audio/webm";
}
...
export async function transcribeAudioWithOpenAI(config: CashClawConfig, args: { mimeType?: string; name?: string; contentBase64: string; requesterId?: string }, requestId?: string): Promise<{ text: string; model: string }> {
  ...
  const normalizedMimeType = normalizeAudioMimeType(args.mimeType);
  if (!ALLOWED_AUDIO_MIME_TYPES.has(normalizedMimeType)) {
    throw new Error(`Audio type ${normalizedMimeType} is not supported for transcription.`);
  }
  ...
  if (buffer.length > MAX_AUDIO_TRANSCRIPTION_BYTES) {
    throw new Error(`Audio recording exceeds the ${MAX_AUDIO_TRANSCRIPTION_BYTES} byte limit.`);
  }
  ...
  for (const currentModel of models) {
    try {
      ...
      form.set("file", new Blob([buffer], { type: normalizedMimeType }), args.name?.trim() || "cateo-audio.webm");
      ...
      const response = await fetchWithTimeout("https://api.openai.com/v1/audio/transcriptions", { method: "POST", headers: { Authorization: `Bearer ${config.llm.apiKey}` }, body: form }, TRANSCRIPTION_TIMEOUT_MS);
      if (!response.ok) throw new Error(`OpenAI transcription failed for ${currentModel} with status ${response.status}`);
