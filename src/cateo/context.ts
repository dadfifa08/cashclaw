...
export function buildCateoContext(caseId: string, input: CateoAssistInput, attachmentEvidence: CateoAttachmentEvidence[] = [], partResolution?: CateoPartResolution | null): CateoContextBundle {
  ...
  const contextSummary = uniqueStrings([
    ...
    partResolution?.referenceDocuments.length ? `Reference documents identified: ${partResolution.referenceDocuments.slice(0, 4).join(", ")}.` : undefined,
    partResolution?.hazardSignals.length ? `Hazard and warning labels identified: ${partResolution.hazardSignals.slice(0, 4).join("; ")}.` : undefined,
    ...((partResolution?.expectedValues ?? []).slice(0, 4)).map((value) => `Expected value reference: ${value}`),
    ...((partResolution?.groundedFindings ?? []).slice(0, 4)),
