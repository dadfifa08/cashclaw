...
export function listDeletedPublicUsers(config: CashClawConfig) { return loadUsers().users.filter((entry) => entry.status !== "active").sort((a, b) => b.updatedAt.localeCompare(a.updatedAt)).map((entry) => userSnapshot(config, entry)); }
